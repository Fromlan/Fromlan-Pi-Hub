...
│   ├── agents-store.ts        # agent 元数据 JSON 持久化（{userData}/lite-pi/agents.json）
│   └── persistence.ts         # 消息历史与已停止会话快照持久化（{userData}/lite-pi/）
...
读写 ~/.pi/agent/{prompts,skills,extensions}/ 与 ~/.pi/agents/<name>/{prompts,skills,extensions}/，对应 IPC 通道 `plugin:list/read/save/create/delete` 与 `agent:list/create/delete` + `agent:file:*`。保存/删除后主进程 broadcast 对应 `*Changed` 事件，渲染端面板按需刷新 + 提示"在任意 Pi 会话中执行 `/reload` 以应用更改"。**Lite-Pi 是 RPC 客户端，无远程触发 `/reload` 的能力**，不自动注入（会打断活跃会话）。
