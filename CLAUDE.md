...
Squads / Autopilots / Inbox /             issue-runner + task-monitor       (JSONL 协议)
Composer / MessageList /                  broadcast 事件 →
store.applyEvent 增量拼流                plugin/agent/squad/autopilot/
store.setIssues / setTasks               inbox/settings stores + persistence
                                         (env 白名单 + path/symlink 守卫)
```

**导航状态**：`activePanel`（chat | agents | plugins | settings | squads | autopilots | inbox）+ `viewMode`（kanban | list | session）+ `activeSessionId` / `activePersistedId` / `activeIssueId`。
...
└── renderer/
    ...
    └── components/
        ...
        ├── SquadsPanel.tsx / AutopilotsPanel.tsx / InboxPanel.tsx
...
持久化根目录：`{userData}/fromlan-pi-hub/`（issues / comments / tasks / squads / autopilots / inbox / settings / sessions / messages）。
...
路由状态走 `activePanel + viewMode + activeSessionId + activePersistedId + activeIssueId`。
