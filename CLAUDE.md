...
```bash
# 1. 改 package.json version（例如 0.9.2）并 commit
...
│   ├── issue-runner.ts        # Issue→Task→Session 派活、Leader/mention/cron、失败重试、Skill 提炼钩子
│   ├── resume-policy.ts       # 毒化 session / workdir resume 门闸（Multica 本地切片）
│   ├── skill-extract.ts       # 解析 fromlan-skill 围栏并写入 skills/
│   ├── hub-agent-bridge.ts    # 127.0.0.1 HTTP 桥：hub_* 工具 → issue-store
│   ├── hub-issue-extension.ts # 将 hub-issue.ts 落到 userData，供 --extension 注入
...
### Skill 自动提炼
- 成功 `onSessionCompleted`（非 squad_leader）解析摘要中的 `fromlan-skill` 围栏 → `skill-extract.ts` 写入 `~/.pi/agents/<name>/skills/`（或全局）。
- `AppSettings.skillExtractMode`：`off` | `propose`（默认，写盘 + Inbox `skill_proposed`）| `auto`。
- 同名冲突自动加 `-2` 后缀；评论写回前剥离围栏。

### Agent hub_* 工具（主动 PM）
- 带 `issueId` 的派活 session：注入 `--extension`（`hub-issue-extension.ts` 落到 userData）+ bridge env。
- `hub-agent-bridge.ts` 仅监听 `127.0.0.1`；Bearer 鉴权；校验 `hubSessionId` 对应 running task。
- 工具：`hub_set_status` / `hub_report_blocker` / `hub_create_issue` / `hub_add_comment`。
- 桥写入 `task.agentStatusOverride` 后，`onSessionCompleted` **不再**盲目 `in_progress → in_review`。
...
### Inbox 原则
**Agent 不进 Inbox。** Inbox 只给人（本机用户）看：mention / assign / task_failed / skill_proposed 等。
...
### 借鉴 Multica / 不借鉴
详见 [ROADMAP.md](./ROADMAP.md)。全量借鉴任务模型、超时重试、四触发、Squad、Project 分组、Skill（含成功 run 提炼）、Inbox 给人、毒化 resume 门闸、Agent 主动改 Issue；**不做** Server–Daemon–Cloud、多 CLI、多 Workspace、github_repo Resource、Autopilot webhook、每 task 隔离沙箱目录（沿用 Project `defaultCwd` / session cwd）。
