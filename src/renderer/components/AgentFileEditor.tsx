...
export function AgentFileEditor({ agentName, type, target, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    let alive = true;
    (async () => {
      const r = await window.agentAPI.fileRead(agentName, type, target.name);
      if (!alive) return;
      if ("ok" in r) {
        setError(r.error);
      } else {
  ...
  const remove = async () => {
    ...
    setError(null);
    try {
      const r = await window.agentAPI.fileDelete(agentName, type, name);
      if (!r.ok) throw new Error(r.error);
      onClose();
    } catch (e) {
      setError((e as Error).message);
