# Fromlan Pi Hub 迭代路线图(ROADMAP)

> 这份路线图定义 Fromlan Pi Hub 从 v0.2.0 → v1.0.0 的演进方向、阶段边界与验收标准。
> 设计灵感来自 [Multica](https://github.com/multica-ai/multica),沿用其**任务模型、分派协议、Skill 标准化、Squad 路由**这套已经被 40k stars 验证的设计哲学,但保留 Fromlan Pi Hub "**只服务 pi、纯本地、桌面原生**" 的定位。
>
...
> **让 Fromlan Pi Hub 成为 Windows 桌面上 "Multica 本地单机版"** —— 人管 Agent,Agent 干活的工位台。

Fromlan Pi Hub 不是一个聊天客户端,而是一个"**管 Agent + 分派任务 + 看结果 + 攒 Skill + 配 Cron**"的工作台。用户打开 Fromlan Pi Hub 看到的不再是"消息流",而是"**待办看板**"。

---

## 二、设计灵感来自 Multica

借鉴 Multica 的核心理念(行为与数据模型为主;Issue 看板/详情视觉层级见 [DESIGN.md](./DESIGN.md) §8.6；已引入 **Project** 分组与加深后的 Squad 协调循环，仍不做 Labels):
...
| 6 | **Skill = 知识包** | 对齐 Anthropic Agent Skills 标准的 `SKILL.md` + 附属文件；成功派活可用 `fromlan-skill` 围栏自动提炼。 |
| 7 | **Runtime = Daemon × CLI** | 单一组合,跨 Workspace 即分发(本地单机可跳过)。 |
| 8 | **失败回滚 status** | 任务失败 → issue status 自动 `in_progress → todo`,看板立刻反映。 |
| 9 | **毒化 resume 门闸** | 钉住 prior session + workdir；毒化则复用目录开新会话；workdir 缺失则全新；Issue 级 rerun 始终新会话。 |
| 10 | **Agent 主动 PM** | 派活中可通过工具改状态、报 blocker、建子 Issue（本地 bridge，非云 API）。 |
| 11 | **Usage 看板** | 按 task/session 记 token/cost，按天聚合图表（本机 jsonl，无云 rollup）。 |
| 12 | **订阅切换** | 多套 Provider Profile（含 key）一键写回 `auth.json`（cc-switch 折中，仅服务 pi）。 |

### 借鉴 / 不借鉴硬边界

**全量借鉴**:理念 1、2、3、4、5、6、8、9、10、11（用量模型）。
**折中借鉴**:理念 12（只写 pi auth，不做多 CLI）。
**完全不做**:Multica 的 WebSocket 队列、跨 Workspace、多人协作、16 CLI 支持、Cloud Runtimes、每 task 隔离沙箱 worktree。Fromlan Pi Hub 是本地单机 + 单 CLI,这是定位决定的。

### Fromlan Pi Hub 独有优势(反过来是 Multica 没有的)

- 已落地：每 session 独立 cwd、会话历史快照 + 继续对话、纯本地无云、pi RPC 深度集成（agent 隔离 / env 白名单）
- 仍待兑现的桌面原生：系统托盘、全局快捷键、文件拖放到会话/Issue（看板拖拽状态已有）
...
**目标**:把 Fromlan Pi Hub 从"聊天客户端"升级成"看板 + 详情"双视图。**这是产品形态的认知杠杆**,本阶段后用户的心智负担从"消息流过载"变成"待办清晰"。
...
```
Issue { id, key('FPH-1' 大写 3-10 字母 + 顺序号), title, description,
...
- **顶部切换器**:`看板 / 列表 / 当前会话`,而不是云 Workspace 切换器。Fromlan Pi Hub 是单机,概念更轻。
...
**目标**:Fromlan Pi Hub 真正出圈的功能 —— 让一个 Agent 替人**选谁该干活**,模拟真实团队"组长派活"的协作。
...
- Webhook(暂缓,仅 cron + Run now；不在本路线图强制交付)
...
**目标**:用户可以"开着应用去干别的",任务在背景跑,回到 Fromlan Pi Hub 看通知。
...
**明确不做**:多 CLI 支持。Fromlan Pi Hub 立项本意就是"**只服务 pi**"。Multica 的"16 CLI 支持"是其差异化能力,反过来,**Fromlan Pi Hub 在 "Pi-only 极致体验" 上做文章**。
...
**状态(2026-07-20)**:阶段 0–6(v0.3–v0.9)、**Projects / Squad 加深**、**Multica 缺口三件套**与 **用量 + 订阅 Profile** 均已落地。下一刀是 **v1.0.0 GA**:E2E 验收、发布打包与文档一致性。

历史说明——当初第一刀选阶段 1 的理由仍然成立:看板把产品从"聊天工具"抬成"工作台";后续可靠性 / Squad / Autopilot / Inbox 都建立在 Issue+Task 语义上。
...
| v0.3.0 | 阶段 0 | 2026-07 | 语义对齐、Task 占位（随 v0.4 一并落地） |
| **v0.4.0** | **阶段 1** | **2026-07-17** | Issue + Kanban + Detail + Comment + 薄派活 |
| **v0.5.0** | **阶段 2** | **2026-07-18** | task-monitor 双超时 + 分类重试 + TaskHistory + Settings |
| **v0.6.0** | **阶段 3** | **2026-07-18** | Squad Leader Protocol + `@mention` + MentionPicker |
| **v0.7.0** | **阶段 4** | **2026-07-18** | SKILL.md 提示 + zip 导入 |
| **v0.8.0** | **阶段 5** | **2026-07-18** | Autopilot cron + Run now + 触发日志 |
| **v0.9.0** | **阶段 6** | **2026-07-18** | Inbox（仅人侧）+ Electron 桌面通知 |
| **v0.9.x** | **Projects + Squad 加深** | **2026-07-19** | Project 容器；Squad 再唤醒 / 归档迁移 / role / pick-first-member |
| **v0.9.x** | **Multica 缺口三件套** | **2026-07-20** | Skill 提炼；毒化 resume；Agent hub_* 主动 PM（发布版 **0.9.2**） |
| **v0.9.x** | **用量 + 订阅** | **2026-07-20** | Usage 按天图表；Provider Profile 写 auth.json（cc-switch 折中，仅 pi） |
| v1.0.0 | 整体 | — | GA：E2E 验收、文档与发布打包 |

每次阶段完成,在本表新增一行,标注发布日期、是否完成、是否有 scope 调整。

### v0.9.x 交付(2026-07-20) — Skill 提炼 / 毒化 resume / Agent hub_*

在编排层之上对齐 Multica 本地缺口三件（定位不变：pi-only、无沙箱 worktree、无 webhook）。

**Skill 自动提炼**:
- `skill-extract.ts`；摘要 `fromlan-skill` 围栏；写入 agent/全局 skills
- `AppSettings.skillExtractMode`：`off` | `propose`（默认）| `auto`；Inbox `skill_proposed`
- Squad Leader 完成不提炼；`buildIssuePrompt` 说明围栏格式

**毒化 resume**:
- Task 字段 `piSessionId` / `workdir` / `sessionPoisoned`；错误原因 `session_poisoned`
- `resume-policy.ts` 门闸；自动 retry + TaskHistory `resumeFromTaskId`；Issue 级 rerun 始终新会话
- `--session` 的 `get_state` 失败 → 同 cwd 降级新会话

**Agent 主动 PM**:
- `hub-agent-bridge.ts`（127.0.0.1）+ `hub-issue-extension.ts`（`--extension`）
- 工具：`hub_set_status` / `hub_report_blocker` / `hub_create_issue` / `hub_add_comment`
- `agentStatusOverride` → 完成时跳过盲目 `in_review`；更新 Helper IDENTITY

**文档**:README / CLAUDE / ROADMAP 同步；`npm run typecheck` 双侧 0 error。

下次阶段:**v1.0.0**(E2E 与正式 GA)。

### v0.9.x 交付(2026-07-19) — Projects + Squad Multica 对齐加深

在 v0.9.0 编排层之上补齐 Multica **Projects** 本地切片,并加深 Squad 运行时语义。

**Projects**:
- `project-store.ts` / `projects.json`;Issue `projectId`;IPC `project:*` + `projectAPI`
- `ProjectsPanel` / `ProjectPicker`;看板顶栏项目过滤;`defaultCwd` → `resolveModel` / Leader briefing
- 删项目只 unlink Issue;进度 `done / (linked − cancelled)`
- **不做**:github_repo、侧栏钉选、Labels、多用户权限

**Squad 加深**(对齐 [Multica Squads](https://multica.ai/docs/squads)):
- Issue 已派给 Squad 时的评论再唤醒表(`shouldRetriggerSquadLeader` / `handleCommentTriggers`)
- 归档 = 软删 + Issue 转给 Leader;拒绝派给已归档小队
- 成员 `role` UI;Leader 无 mention → `pickFirstSquadMember`;指派失败 UI 回滚

**文档 / UI**:README / CLAUDE / DESIGN / ROADMAP 同步;`labels.ts`;表单网格与看板「新建 Issue」nowrap 修复。

**Pi Hub Helper**(对齐 Multica Helper,本地化):
- 首次启动种子 `pi-hub-helper` + `IDENTITY.md`;`--append-system-prompt` 注入身份
- 欢迎弹窗三选一起步 Issue(介绍 / 导览 / slides);`settings.onboardedAt` 门控;无 Hub CLI
- 产品简称 **Pi Hub**（全称 Fromlan Pi Hub；勿简称 Fromlan）

**验收**:`npm run typecheck` 双侧 0 error。

（续：2026-07-20 缺口三件套见上方「Multica 缺口三件套」节。）

### v0.5.0–v0.9.0 合并交付(2026-07-18)

按 Multica 编排缺口计划一次落地「编排可信层」,定位不变:pi-only、本地单机、桌面原生。

**主进程新增**:
- `task-monitor.ts` — 5s 扫描;dispatched 超时 / running 超时;调用 `failTaskWithInfo`
- `settings-store.ts` — 默认派活模型、超时、maxRetries、notifyMode
- `squad-store.ts` — Squad CRUD(`squads.json`)
- `autopilot-store.ts` / `autopilot-manager.ts` — cron 调度 + 触发日志
- `inbox-store.ts` / `notification.ts` — Inbox 持久化 + 桌面通知节流
- `issue-runner.ts` 扩展 — Squad Leader prompt、`enqueueForAgent`、mention/cron/retry 触发、失败 Inbox

**共享**:
- `src/shared/mention.ts` — `[@Name](mention://agent|squad|human/<id>)` 解析与生成
- `types.ts` — Task 错误分类、Squad / Autopilot / Inbox / AppSettings、扩展 PanelKind 与 IPC

**渲染**:
- `TaskHistory` / `MentionPicker` / `SquadsPanel` / `AutopilotsPanel` / `InboxPanel`
- IconRail: Squads、Autopilots、Inbox(未读角标)
- IssueDetail: dueDate、parent、@mention、TaskHistory;Settings: 派活默认与可靠性参数
- PluginsPanel: Skill zip 导入;Plugin/Agent 编辑器 SKILL.md 提示条

**依赖**: `node-cron`、`cronstrue`、`adm-zip`

**验收**: `npm run typecheck` 双侧 0 error;`npm run build` 三段产物完整。

**刻意未做**(与计划一致):
- Autopilot webhook
- 多 CLI / 云 Runtime / 多人协作

**已补齐**(原「刻意未做」):
- workdir/session 毒化后的精细 resume（钉住 piSessionId/workdir；毒化则同目录新会话；Issue 级 rerun 始终新会话）
- 从成功 run 摘要 `fromlan-skill` 围栏提炼 Skill（settings: off/propose/auto）
- Agent 经 hub_* 工具主动建 Issue / 报 blocker / 改状态（本机 bridge + 注入 extension）

下次阶段:**v1.0.0**(打磨与正式 GA)。
...
- `src/renderer/components/SessionCard.tsx` —— 若 issueId 关联,显示"来自 FPH-N 任务"标签
...
~~下次阶段:v0.5.0~~ → 已在 2026-07-18 与 v0.5–v0.9 一并交付(见上方合并交付节)。
...
**UI 对齐(2026-07-18)**：Issue 看板/详情视觉对齐 Multica（四行卡片、`PriorityIcon` / `IssueStatusIcon`、280px 列 tint、详情「文档主栏 + Properties 侧栏」、评论 Avatar gutter）。规范写入 [DESIGN.md](./DESIGN.md) §8.6。

下次阶段:**v1.0.0**(打磨与正式 GA)。编排层 v0.5–v0.9 已于 2026-07-18 交付；Projects / Squad 加深与 Multica 缺口三件套见上方 v0.9.x 节。
...
| 持久化目录 | 复用 `{userData}/fromlan-pi-hub/`,新增 `issues.json` / `comments.json` / `tasks.json` / `squads.json` / `autopilots.json` | 单一根,atomic write,迁移成本低 |
...
| Webhook 触发 | 阶段 5 暂缓 | local 单机,内网 webhook 没强需求;先做 cron 验证逻辑（仍未做） |
| 毒化 resume / Skill 提炼 / hub_* | v0.9.x (2026-07-20) | 已落地；不做每 task 沙箱 worktree |
| Cron 解释时区 | **本地时区**(IANA) | 与 Fromlan Pi Hub "Windows 桌面" 强绑定;避免服务器/客户端时区漂移 |
